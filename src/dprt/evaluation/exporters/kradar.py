...
class KRadarExporter:
    def __init__(
        ...
        **kwargs,
        ...
        self.conf_thrs = (
            conf_thrs if conf_thrs is not None else [0.0, 0.3, 0.5, 0.7, 0.9]
        )
    ...
    def _construct_objects(
        self, objects: Dict[torch.Tensor], conf_thr: float, pre: str = ""
    ) -> torch.Tensor:
        ...
        angle = torch.atan2(
            objects[f"{pre}angle"][..., 0], objects[f"{pre}angle"][..., 1]
        )
        ...
        x_mask = (0 < objects[f"{pre}center"][:, 0]) & (
            objects[f"{pre}center"][:, 0] < 72
        )
        y_mask = (-6.4 < objects[f"{pre}center"][:, 1]) & (
            objects[f"{pre}center"][:, 1] < 6.4
        )
        z_mask = (-2.0 < objects[f"{pre}center"][:, 2]) & (
            objects[f"{pre}center"][:, 2] < 6.0
        )
        ...
        return torch.hstack(
            [
                ...
                torch.zeros((N, 1), dtype=int, device=device),
                torch.tensor([[50, 50, 150, 150]], dtype=int, device=device).repeat(
                    N, 1
                ),
    ...
    def _export_output_objects(
        self,
        objects: Dict[torch.Tensor],
        conf_thr: float,
        step: int,
    ...
    def _export_target_objects(
        ...
        for desc in itertools.chain(["all"], description):
            ...
            self.write(
                description, osp.join(folder, "desc", f"{str(step).zfill(6)}.txt")
            )
    ...
    def _export_output_batch(
        ...
        """
        # Decollate batch
        batch: List[Dict[str, torch.Tensor]] = decollate_batch(
            batch, detach=True, pad=False
        )
